...
Feature: Issuing one token
  ...
  Scenario: Customer with at least one token tries to get one token issued
     Given a registered customer with id "9b6b1fd5-e37f-479d-a262-6533405e781d" with at least 1 token
     When the event CustomerTokensRequest "CustomerTokensRequest" is sent
     Then a response CustomerTokensReturned "CustomerTokensReturned" is sent and a customer receives a token
